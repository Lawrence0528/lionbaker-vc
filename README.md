...
本系統是一個「AI 產碼 → 資料承接 → Sandbox 執行驗證」的一體化工作台，協助使用者用模板化提示詞驅動 Gemini 生成程式，並在平台內完成資產管理與執行測試。

### 核心能力
- 提示詞模板：提供多種模板將需求結構化後送至 Gemini，產出可交付的程式碼/頁面內容。
- 資產與資料承接：
  - Firebase Storage 作為圖床與專案資產儲存。
  - Firestore 作為專案與畫面輸入資料（Datastore）承接層，用於保存專案設定、內容與狀態。
- Sandbox：提供可執行環境，讓客戶/使用者將生成後的程式碼放入並運行，用於驗證與展示。
---
